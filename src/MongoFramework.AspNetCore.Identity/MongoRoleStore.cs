...
namespace MongoEntityFramework.AspNetCore.Identity
{
        ...
        where TRoleClaim : IdentityRoleClaim<TKey>, new()
    {
        ...
        public virtual Task<IList<Claim>> GetClaimsAsync(TRole role, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return Task.FromResult<IList<Claim>>(role?.Claims?.Select(claim => claim.ToClaim()).ToList() ?? new List<Claim>());
        }
        ...
        public virtual async Task RemoveClaimAsync(TRole role, Claim claim, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            Check.NotNull(role, nameof(role));
            Check.NotNull(claim, nameof(claim));
            var claims = role.Claims.Where(uc => uc.RoleId.Equals(role.Id) && uc.ClaimValue == claim.Value && uc.ClaimType == claim.Type).ToList();
